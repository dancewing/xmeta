...
import static io.xmeta.core.sqlbuilder.Predicates.eq;
 ...
 *
 * Each mapping must be provided with an {@link MetaRuntime} object representing
...
public class Mapping<T> {
    ...
    public class Delete {
        ...
        private Delete() {
            this.delete = new DeleteCreator(entity);
        }
...
//        }
        ...
        public int delete() {
            return new NamedParameterJdbcTemplate(metaRuntime.getDataSource()).update(delete.getSQL(), delete.createSqlParameterSource());
        }
    ...
    public class Query {
        ...
        private final SelectCreator select;

        private Query(Entity entity) {
            this.select = new SelectCreator(entity).from(table + " " + alias);
        }
        ...
        public List<T> getResultList() {
            ...
            return new NamedParameterJdbcTemplate(metaRuntime.getDataSource()).query(select.getSQL(), select.createSqlParameterSource(), new RowMapper<T>() {
                @Override
    ...
    private MetaRuntime metaRuntime;
    ...
    public Mapping(MetaRuntime metaRuntime, Class<T> clazz, String table) {
        this.metaRuntime = metaRuntime;
        this.clazz = clazz;
        this.table = table;
    }

    public Mapping(MetaRuntime metaRuntime, Class<T> clazz, Entity entity) {
        this.metaRuntime = metaRuntime;
        this.clazz = clazz;
        this.entity = entity;
    ...
    public Query findWhere(Predicate predicate) {
        return new Query(this.entity).where(predicate);
    }
    ...
    private Converter<?> getConverter(Column column) {
        if (column.getConverter() != null) {
            ...
            return metaRuntime.getConverterFactory().getConverter(field.getType());
        }
    }
    ...
    public T insert(T entity) {
        ...
        InsertCreator insert = new InsertCreator(this.entity);
        ...
        new NamedParameterJdbcTemplate(metaRuntime.getDataSource()).update(insert.getSQL(), insert.createSqlParameterSource());
    ...
    public T update(T entity) throws RowNotFoundException, OptimisticLockException {
        ...
        UpdateCreator update = new UpdateCreator(this.entity);
        ...
        int rows = new JdbcTemplate(metaRuntime.getDataSource()).update(update.getSQL());
        ...
        if (rows == 1) {
            ...
            SelectCreator selectById = new SelectCreator(this.entity)
            .column("count(*)")
            .from(table)
            .whereEquals(idColumn.getColumnName(), getPrimaryKey(entity));

            rows = new JdbcTemplate(metaRuntime.getDataSource()).query(selectById.getSQL(), new ResultSetExtractor<Integer>() {
                @Override
